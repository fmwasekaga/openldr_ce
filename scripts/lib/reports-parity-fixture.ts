...
export const PARAM_BAG: Record<string, unknown> = {
  from: '2026-01-01',
  to: '2026-12-31',
  facility: '',
  asOf: '',
...
export function firstDiff(a: Record<string, unknown>[], b: Record<string, unknown>[]): Diff | null {
  if (a.length !== b.length) {
    return { reason: `row count mismatch: a=${a.length} b=${b.length}`, a: a.slice(0, 3), b: b.slice(0, 3) };
  }
  for (let i = 0; i < a.length; i++) {
    const x = JSON.stringify(a[i]); const y = JSON.stringify(b[i]);
    if (x !== y) return { reason: `row ${i} differs`, a: a[i], b: b[i] };
  }
  return null;
}
