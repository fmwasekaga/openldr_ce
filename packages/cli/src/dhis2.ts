...
// SP-4 changed createDhis2Context to require { loadSink }, sourced from the app context's
// plugin runtime (mirrors apps/server: createDhis2Context(cfg, { loadSink: ctx.plugins.loadSink })).
async function openDhis2(): Promise<{
  app: Awaited<ReturnType<typeof createAppContext>>;
  ctx: Awaited<ReturnType<typeof createDhis2Context>>;
  close: () => Promise<void>;
}> {
  const cfg = loadConfig();
  const app = await createAppContext(cfg);
  const ctx = await createDhis2Context(cfg, {
    loadSink: (id, version) => app.plugins.loadSink(id, version),
  });
  return {
    app,
    ctx,
...
export async function runDhis2MapImport(file: string, opts: { json: boolean }): Promise<number> {
  const { ctx, close } = await openDhis2();
  try {
...
export async function runDhis2Push(mappingId: string, opts: { period: string; dryRun: boolean; json: boolean }): Promise<number> {
  const { app, ctx, close } = await openDhis2();
  try {
