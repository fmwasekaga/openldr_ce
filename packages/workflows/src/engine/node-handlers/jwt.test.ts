...
// HS256 wants a key of at least 32 bytes — use realistic-length secrets.
const SECRET = 'abcdefghijklmnopqrstuvwxyz0123456789';
const WRONG = 'ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ';

describe('jwtHandler', () => {
  it('signs then verifies a payload round-trip', async () => {
    const signed = await jwtHandler(node({ operation: 'sign', secret: SECRET, algorithm: 'HS256', payloadField: 'claims', outputField: 'token' }), ctx(), [{ json: { claims: { sub: 'u1', role: 'admin' } } }]);
    const token = (signed[0].json as Record<string, unknown>).token as string;
    expect(typeof token).toBe('string');
    expect(token.split('.')).toHaveLength(3);
    const verified = await jwtHandler(node({ operation: 'verify', secret: SECRET, tokenField: 'token', outputField: 'payload' }), ctx(), [{ json: { token } }]);
  ...
  it('marks an invalid signature as not valid', async () => {
    const signed = await jwtHandler(node({ operation: 'sign', secret: SECRET, payloadField: 'claims', outputField: 'token' }), ctx(), [{ json: { claims: { a: 1 } } }]);
    const token = (signed[0].json as Record<string, unknown>).token as string;
  ...
  it('decodes without verifying', async () => {
    const signed = await jwtHandler(node({ operation: 'sign', secret: SECRET, payloadField: 'claims', outputField: 'token' }), ctx(), [{ json: { claims: { hello: 'world' } } }]);
    const token = (signed[0].json as Record<string, unknown>).token as string;
