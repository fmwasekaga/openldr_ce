...
export const sampleWorkflow: Workflow = {
  ...
  definition: {
    nodes: [
      ...
      { id: 'if-1', type: 'condition', position: { x: 540, y: 260 }, data: { label: 'Is premium?', condition: '$input.data && $input.data.id > 0', templateId: 'if', iconName: 'GitBranch' } },
      { id: 'log-1', type: 'action', position: { x: 720, y: 140 }, data: { label: 'Log premium', action: 'log', message: 'Premium user: {{ $input.data.name }}', level: 'info', config: {}, templateId: 'log', iconName: 'Terminal' } },
      ...
      { id: 'code-1', type: 'code', position: { x: 1040, y: 140 }, data: { label: 'Build report', code: 'console.log("Building report for", $input);\nreturn { report: "done", ts: Date.now() };', language: 'javascript', templateId: 'code', iconName: 'Code' } },
      ...
      { id: 'filter-1', type: 'condition', position: { x: 1400, y: 260 }, data: { label: 'Has data?', condition: '$input && $input.length > 0', templateId: 'filter', iconName: 'Filter' } },
