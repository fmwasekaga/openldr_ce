...
describe('activity routes', () => {
  it('GET /api/activity returns the recent list', async () => {
    ...
    const app = appWithUser(['data_analyst'], (a) => registerActivityRoutes(a, ctx));
  ...
  it('GET /api/activity honours limit/offset query params', async () => {
    const { ctx, calls } = fakeCtx();
    const app = appWithUser(['lab_admin'], (a) => registerActivityRoutes(a, ctx));
  ...
  it('GET /api/activity/:id returns the lifecycle when found', async () => {
    const { ctx } = fakeCtx();
    const app = appWithUser(['system_auditor'], (a) => registerActivityRoutes(a, ctx));
  ...
  it('GET /api/activity/:id returns 404 when unknown', async () => {
    const { ctx } = fakeCtx();
    const app = appWithUser(['lab_manager'], (a) => registerActivityRoutes(a, ctx));
