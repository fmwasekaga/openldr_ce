...
const { authFetch } = vi.hoisted(() => ({ authFetch: vi.fn(async () => ({ ok: true, status: 200, json: async () => [] })) }));
...
describe('reportDefsApi', () => {
  ...
  it('creates via POST', async () => {
    authFetch.mockResolvedValueOnce({ ok: true, status: 201, json: async () => ({ id: 'r1' }) });
  ...
  it('deletes via DELETE', async () => {
    authFetch.mockResolvedValueOnce({ ok: true, status: 204, json: async () => null });
