    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="w-full max-w-lg max-h-[80vh] flex flex-col p-0">
        <div className="border-b border-border px-6 py-4">
          <DialogTitle className="text-base font-semibold">{t('reportBuilder.parameters.title')}</DialogTitle>
        </div>
            ...
            <div key={i} className="space-y-2 rounded-md border border-border p-3">
              <div className="grid grid-cols-3 gap-2">
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('reportBuilder.parameters.variableId')}</Label>
                  <Input aria-label={`param-${i}-id`} className="h-8 text-xs" value={p.id}
                    onChange={(e) => update(i, { id: e.target.value.replace(/[^a-zA-Z0-9_]/g, '') })} />
                </div>
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('reportBuilder.parameters.label')}</Label>
                  <Input aria-label={`param-${i}-label`} className="h-8 text-xs" value={p.label}
                    onChange={(e) => update(i, { label: e.target.value })} />
                </div>
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('reportBuilder.parameters.type')}</Label>
              ...
              {p.type === 'select' && (
                <div className="space-y-1">
                  <Label className="text-[10px] uppercase tracking-wide text-muted-foreground">{t('reportBuilder.parameters.optionsSql')}</Label>
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="flex items-center gap-1 text-xs text-muted-foreground">
                  <input type="checkbox" aria-label={`param-${i}-required`} checked={p.required}
                    onChange={(e) => update(i, { required: e.target.checked })} />{t('reportBuilder.parameters.required')}
                </label>
