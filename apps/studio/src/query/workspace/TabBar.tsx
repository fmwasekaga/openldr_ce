...
export function TabBar(): JSX.Element {
  ...
  return (
    <div className="flex items-end gap-1 border-b border-border bg-muted/40 pl-3 pr-2 pt-2">
      {tabs.map((t) => {
        const active = t.id === activeId;
        return (
          <div key={t.id}
            className={`relative flex items-center gap-2 rounded-t-lg border px-3.5 py-2 text-xs ${active
              ? '-mb-px z-10 border-border border-b-background bg-background text-foreground'
