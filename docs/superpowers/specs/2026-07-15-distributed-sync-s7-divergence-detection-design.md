...
- **The detail and clear routes check-then-act (a benign TOCTOU).** Both `getDivergence` then act. Two concurrent clears would each see the row, each issue the idempotent `DELETE ... WHERE` (the second is a no-op, no error), and each write an audit event — a **duplicate audit row, not data loss**. Fixing it means widening `SyncDivergenceStore.clear()` to return an affected-row count. Deferred deliberately: the routes are `lab_admin`-gated and human-driven, not a bulk/automated caller. Logged here rather than fixed.
